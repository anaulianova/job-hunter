...
But applying is not just tailoring your CV, it's answer all those "Why do you want to work at this company?" questions. So when you pick a role and decide to apply to it, job-hunter will generates answers to every standard application question — salary expectations, why this company, note to the hiring manager — with tone calibrated to the role tier so you can save time. An internal reviewer pass simulates a hiring manager skimming your answers and tightens anything that wouldn't survive a skim. You validate, confirm, and the answers sync to a Google Sheets tracker automatically.
...
/apply
  └── Q&A generation (salary, why this company, note to HM),
      internal reviewer pass, user validation, sync to Google Sheets.
        │
        ▼
