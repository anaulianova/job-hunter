...
From there, the workflow runs like this: you feed it a job description (URL, pasted text, or a saved file), and it runs a **five-point evaluation** — suitability check, fitness score out of 100, missing keywords, ATS audit with line-by-line CV rewrites, sensitive role decisions, and a tailored About Me. The CV amendments are exact copy, not suggestions. You paste them in directly.

When you decide to apply, it generates answers to every standard application question — salary expectations, why this company, note to the hiring manager — with tone calibrated to the role tier. Then an internal reviewer pass simulates a hiring manager skimming your answers and tightens anything that wouldn't survive a skim. You validate, confirm, and the answers sync to a Google Sheets tracker automatically.
